...
useEffect(() => {
  ...
  const getExams = async () => {
    try {
      const { data } = await axios.get(`https://quizwhizz.onrender.com/exam/exam/${id.id}`, {
...
const handleReview = () => {
    ...
    axios.put(`https://quizwhizz.onrender.com/userexams/${userexamid}`, userOptions,{ headers: { Authorization: `Bearer ${token}` } }).then((response) => {
...
const handleCapture = () => {
 ...
 const imageDataURL = canvas.toDataURL('image/jpeg'); // You can specify the image format (e.g., 'image/jpeg', 'image/png')
    ...
    console.log(userOptions)
    axios.put(`https://quizwhizz.onrender.com/userexams/proctore/${userexamid}`, userOptions,{ headers: { Authorization: `Bearer ${token}` } }).then((response) => {
      console.log(response.status);
      console.log(response.data);
    });
...
console.log(userexamid);
  axios.put(`https://quizwhizz.onrender.com/userexams/score/${userexamid}`, scorePayload, {
    headers: { Authorization: `Bearer ${token}` }
  })
  .then((response) => {
