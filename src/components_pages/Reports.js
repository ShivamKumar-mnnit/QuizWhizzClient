...
const token = localStorage.getItem('token');
  ...
  const getUserDatas = async () => {
    const { data } = await axios.get(`https://quizwhizz.onrender.com/userexams/`+CUId.CUId,{ headers: { Authorization: `Bearer ${token}` } });
    console.log(data);
    setUserDatas(data)
    setIsLoading(false);
  }
  ...
  const getExamDatas = async () => {
    await axios.get(`https://quizwhizz.onrender.com/exam`,{ headers: { Authorization: `Bearer ${token}` } }).then((response) => {
