...
const Result = () => {
  ...
  const getExamNames = async () => {
    const { data } = await axios.get(`https://quizwhizz.onrender.com/userexams/exam/${id.id}`,{ headers: { Authorization: `Bearer ${token}` } });
    console.log(data);
    setScore(data);
    getPassGrade();
  }
  ...
  const getPassGrade = async () => {
    await axios.get(`https://quizwhizz.onrender.com/exam/exam/${id.id}`,{ headers: { Authorization: `Bearer ${token}` } }).then((response) => {
